<template>
  <div class="container mx-auto">
    <Nav />
    <section v-if="products && products.length" class="text-gray-600 body-font">
...
export default {
  ...
  methods: {
    async getProducts() {
      await this.$axios
        .get(
          `http://192.168.1.12:8077/Product/search/1/${this.perpage}?name=${this.name}&page_no=${this.page_no}`
        )
        .then((response) => {
